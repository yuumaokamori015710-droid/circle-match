...
<body>
  ...
  <main>
    ...
    <section class="section panel"><div class="panel-head"><div><h2>地域から探す</h2><p>地域を押すと、募集掲示板とDB候補をその地域で絞り込めます。</p></div></div><div class="sport-grid" id="regionGrid"></div></section>
  </main>
  ...
  <script>
    ...
    function renderRegions(circles,matches){$("regionGrid").innerHTML=regions.map(r=>{const db=circles.filter(c=>r.prefectures.includes(c.prefecture)).length; const open=matches.filter(m=>r.prefectures.includes(m.prefecture)).length; return `<a class="sport-card region-card" href="/?region=${encodeURIComponent(r.value)}#matches"><span class="region-map">${esc(r.label.slice(0,1))}</span><span class="sport-copy"><strong>${esc(r.label)}</strong><em>DB ${db}件 / 募集 ${open}件</em></span><b>地域で探す</b></a>`}).join("")}
