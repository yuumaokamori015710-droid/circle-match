...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:#f4f7fa;color:var(--ink);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;letter-spacing:0}
    ...
    .db-tools{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;padding:13px 14px;border-bottom:1px solid var(--line);background:#fff}.db-tools strong{font-size:20px}.tablewrap{overflow:auto;max-height:430px}table{width:100%;border-collapse:collapse;font-size:14px;min-width:720px}th,td{padding:11px 12px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}th{position:sticky;top:0;background:#f8fbfd;color:var(--muted);font-size:12px}.name{font-weight:900}.sub{display:block;color:var(--muted);font-size:12px;margin-top:3px}
...
<body>
  ...
  <main>
    ...
    <section id="circleDb" class="section panel"><div class="panel-head"><div><h2>サークルDBを検索</h2><p>大学名、団体名、競技、地域で候補サークルを直接探せます。</p></div><a class="button light" id="dbFullLink" href="/circles">一覧ページで見る</a></div><div class="filters"><input id="dbQ" placeholder="大学名・団体名で検索"><select id="dbRegionFilter"><option value="">全地域</option></select><select id="dbPrefFilter"><option value="">全都道府県</option></select><select id="dbSportFilter"><option value="">全競技</option></select><select id="dbStatusFilter"><option value="">全ステータス</option><option value="university_verified">公式確認済み</option><option value="admin_verified">運営確認済み</option><option value="claimed">申請済み</option><option value="unverified">未確認</option></select></div><div class="db-tools"><span><strong id="dbCount">0</strong> 件のDB候補</span><span class="sub">上位50件を表示します。</span></div><div class="tablewrap"><table><thead><tr><th>大学</th><th>団体名</th><th>競技</th><th>種別</th><th>出典</th></tr></thead><tbody id="dbRows"></tbody></table></div></section>
    <section id="matches" class="section panel"><div class="panel-head"><div><h2>練習試合・交流募集</h2><p>地域、都道府県、競技、大学名、団体名で絞り込めます。募集中が少ない時は、その条件のDB候補へ広げられます。</p></div></div><div class="filters"><input id="q" placeholder="大学名・団体名・場所で検索"><select id="regionFilter"><option value="">全地域</option></select><select id="prefFilter"><option value="">全都道府県</option></select><select id="sportFilter"><option value="">全競技</option></select><select id="typeFilter"><option value="">全募集</option><option>練習試合</option><option>合同練習</option><option>助っ人募集</option><option>大会参加者募集</option></select></div><div id="matchList" class="match-grid" aria-live="polite"></div><div class="section-link"><a id="dbBridge" href="/circles">同じ条件でサークルDBを見る</a></div></section>
  </main>
  ...
  <script>
    ...
    function fillRegions(){ const html='<option value="">全地域</option>'+regions.map(r=>`<option value="${esc(r.value)}">${esc(r.label)}</option>`).join(""); $("regionFilter").innerHTML=html; $("dbRegionFilter").innerHTML=html }
    ...
    function selectedDbRegion(){return regions.find(r=>r.value===$("dbRegionFilter").value)}
    function dbPrefValues(){return selectedDbRegion()?.prefectures || prefs}
    function syncDbPrefOptions(){const current=$("dbPrefFilter").value; fillSelect($("dbPrefFilter"),dbPrefValues(),selectedDbRegion()?`${selectedDbRegion().label}すべて`:"全都道府県"); if(dbPrefValues().includes(current)) $("dbPrefFilter").value=current}
    function dbQuery(){const qs=new URLSearchParams({q:$("dbQ").value,prefecture:$("dbPrefFilter").value,sport:$("dbSportFilter").value,status:$("dbStatusFilter").value}); if($("dbRegionFilter").value) qs.set("region",$("dbRegionFilter").value); return qs}
    ...
    function dbRow(c){return `<tr><td><span class="name">${esc(c.university_name)}</span><span class="sub">${esc(c.prefecture)}${c.city?` / ${esc(c.city)}`:""}</span></td><td><span class="name">${esc(c.circle_name)}</span></td><td>${esc(c.sport_category||"その他")}</td><td>${badge(c.organization_type||"不明")}</td><td>${badge(sourceLabel(c.source_type))}${c.source_url?`<span class="sub"><a href="${esc(c.source_url)}" target="_blank">出典URL</a></span>`:""}</td></tr>`}
    function sourceLabel(v){return ({university_official:"大学公式",self_registered:"本人登録",public_sns:"SNS等",other:"その他"}[v] || v)}
    async function refreshDb(){const qs=dbQuery(); $("dbFullLink").href="/circles?"+qs; const data=await api("/api/circles?"+qs); $("dbCount").textContent=data.length; $("dbRows").innerHTML=data.slice(0,50).map(dbRow).join("") || `<tr><td colspan="5" class="empty">データなし</td></tr>`}
    ...
    async function boot(){renderSports(); fillRegions(); fillSelect($("sportFilter"),sports,"全競技"); fillSelect($("dbSportFilter"),sports,"全競技"); $("regionFilter").value=params.get("region")||""; $("dbRegionFilter").value=params.get("region")||""; syncPrefOptions(); syncDbPrefOptions(); $("q").value=params.get("q")||""; $("dbQ").value=params.get("q")||""; $("prefFilter").value=params.get("prefecture")||""; $("dbPrefFilter").value=params.get("prefecture")||""; $("sportFilter").value=params.get("sport")||""; $("dbSportFilter").value=params.get("sport")||""; await Promise.all([refresh(),refreshDb()])}
    ...
    ["dbQ","dbSportFilter","dbPrefFilter","dbStatusFilter"].forEach(id=>$(id).addEventListener("input",refreshDb));
    $("dbRegionFilter").addEventListener("input",()=>{syncDbPrefOptions(); refreshDb()});
...
<body>
  <header><div class="top"><a class="brand" href="/">__SITE_NAME__</a><nav class="nav"><a class="find-link" href="/">募集を探す</a><a href="/representative">サークル代表はこちら</a><a href="/signin">ログイン</a></nav></div></header>
  <main>
    ...
    <section class="grid"><aside class="panel"><h2>地域別の交流募集</h2><div id="regionList" class="region-list"></div><div id="areaList" class="area-list"></div></aside><section class="panel"><h2>__SPORT__サークルDB</h2><div class="tablewrap"><table><thead><tr><th>大学</th><th>団体名</th><th>種別</th><th>検証</th><th>出典</th></tr></thead><tbody id="circleRows"></tbody></table></div></section></section>
  </main>
