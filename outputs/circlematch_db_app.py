...
<body>
  ...
  <script>
    ...
    async function refresh(){const qs=currentQuery(); $("dbBridge").href="/circles?"+qs; const all=await api("/api/matches?"+qs); const circles=await api("/api/circles?"+qs); const data=sortMatches(all.filter(matchesFilter)); updateStats(circles,data); $("matchList").innerHTML=data.map(card).join("") || `<div class="empty">現在公開中の募集はありません。同じ条件のDB候補は ${circles.length} 件あります。下のDBリンクから候補団体を確認できます。</div>`}
...
<body>
  ...
  <main>
    ...
    <section class="grid"><aside class="panel"><h2>地域別の交流募集</h2><div id="regionList" class="region-list"></div><div id="areaList" class="area-list"></div></aside><section class="panel"><h2>__SPORT__サークルDB</h2><div class="table-controls"><input id="circleSearch" placeholder="大学名・団体名で検索"><select id="circleTypeFilter"><option value="">全種別</option><option>体育会</option><option>部活</option><option>公認サークル</option><option>同好会</option><option>学生団体</option><option>不明</option></select><select id="circleStatusFilter"><option value="">全検証</option><option value="university_verified">公式確認済み</option><option value="admin_verified">運営確認済み</option><option value="claimed">申請済み</option><option value="unverified">未確認</option></select><select id="circleSortFilter"><option value="university">大学名順</option><option value="circle">団体名順</option><option value="type">種別順</option><option value="status">検証順</option></select></div><div class="table-tools"><span><strong id="visibleCircleCount">0</strong> 件を表示</span><span>地域・都道府県は左側で絞り込み</span></div><div class="tablewrap"><table><thead><tr><th>大学</th><th>団体名</th><th>種別</th><th>検証</th><th>出典</th></tr></thead><tbody id="circleRows"></tbody></table></div></section></section>
  </main>
  ...
  <script>
    ...
    function badge(v,cls=""){return `<span class="badge ${cls}">${esc(v)}</span>`}
    async function api(path){const r=await fetch(path); if(!r.ok)throw new Error(await r.text()); return r.json()}
    function sortCircles(rows){const v=$("circleSortFilter").value; return rows.slice().sort((a,b)=>{if(v==="circle")return String(a.circle_name||"").localeCompare(String(b.circle_name||""),"ja"); if(v==="type")return String(a.organization_type||"").localeCompare(String(b.organization_type||""),"ja"); if(v==="status")return String(a.verification_status||"").localeCompare(String(b.verification_status||""),"ja"); return String(a.university_name||"").localeCompare(String(b.university_name||""),"ja")})}
    function renderCircleRows(){const q=$("circleSearch").value.trim().toLowerCase(); const type=$("circleTypeFilter").value; const status=$("circleStatusFilter").value; const filtered=sortCircles(currentCircles.filter(c=>{const blob=[c.university_name,c.circle_name,c.prefecture,c.city,c.organization_type,c.verification_status].join(" ").toLowerCase(); if(q&&!blob.includes(q))return false; if(type&&c.organization_type!==type)return false; if(status&&c.verification_status!==status)return false; return true})); $("visibleCircleCount").textContent=filtered.length; $("circleRows").innerHTML=filtered.map(c=>`<tr><td><span class="name">${esc(c.university_name)}</span><span class="sub">${esc(c.prefecture)} ${esc(c.city||"")}</span></td><td><span class="name">${esc(c.circle_name)}</span></td><td>${badge(c.organization_type||"不明")}</td><td>${badge(statusLabel(c.verification_status),["admin_verified","university_verified"].includes(c.verification_status)?"ok":"")}</td><td>${badge(sourceLabel(c.source_type))}${c.source_url?`<span class="sub"><a href="${esc(c.source_url)}" target="_blank">出典URL</a></span>`:""}</td></tr>`).join("") || `<tr><td colspan="5" class="empty">データなし</td></tr>`}
    ...
    ["circleSearch","circleTypeFilter","circleStatusFilter","circleSortFilter"].forEach(id=>$(id).addEventListener("input",renderCircleRows));
    boot().catch(e=>alert(e.message));
  </script>
</body>
</html>"""
