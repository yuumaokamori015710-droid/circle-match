...
<body>
  ...
  <script>
    ...
    let currentCircles = [];
    let columnFilters = {university:"", circle:"", type:"", status:"", source:"", sort:"university"};
    const $ = id => document.getElementById(id);
    function esc(v){return String(v ?? "").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\\"":"&quot;","'":"&#039;"}[c]))}
    ...
    function badge(v,cls=""){return `<span class="badge ${cls}">${esc(v)}</span>`}
    async function api(path){const r=await fetch(path); if(!r.ok)throw new Error(await r.text()); return r.json()}
    function rowValue(c,key){return key==="university"?`${c.university_name||""} ${c.prefecture||""} ${c.city||""}`:key==="circle"?c.circle_name||"":key==="type"?c.organization_type||"不明":key==="status"?c.verification_status||"":key==="source"?c.source_type||"":""}
    function displayValue(key,value){if(!value)return "すべて"; if(key==="status")return statusLabel(value); if(key==="source")return sourceLabel(value); return value}
    function sortCircles(rows){const v=columnFilters.sort; return rows.slice().sort((a,b)=>{if(v==="circle")return String(a.circle_name||"").localeCompare(String(b.circle_name||""),"ja"); if(v==="type")return String(a.organization_type||"").localeCompare(String(b.organization_type||""),"ja"); if(v==="status")return String(a.verification_status||"").localeCompare(String(b.verification_status||""),"ja"); if(v==="source")return String(a.source_type||"").localeCompare(String(b.source_type||""),"ja"); return String(a.university_name||"").localeCompare(String(b.university_name||""),"ja")})}
    function updateFilterButtons(){document.querySelectorAll("[data-filter]").forEach(b=>{const key=b.dataset.filter; b.classList.toggle("active",!!columnFilters[key]||columnFilters.sort===key)}); const labels={university:"大学",circle:"団体名",type:"種別",status:"検証",source:"出典"}; const active=Object.keys(labels).filter(k=>columnFilters[k]).map(k=>`${labels[k]}: ${displayValue(k,columnFilters[k])}`); $("activeFilterText").textContent=active.length?active.join(" / "):"列見出しの漏斗で絞り込み"}
    function renderCircleRows(){const filtered=sortCircles(currentCircles.filter(c=>{if(columnFilters.university&&!rowValue(c,"university").toLowerCase().includes(columnFilters.university.toLowerCase()))return false; if(columnFilters.circle&&!rowValue(c,"circle").toLowerCase().includes(columnFilters.circle.toLowerCase()))return false; if(columnFilters.type&&rowValue(c,"type")!==columnFilters.type)return false; if(columnFilters.status&&rowValue(c,"status")!==columnFilters.status)return false; if(columnFilters.source&&rowValue(c,"source")!==columnFilters.source)return false; return true})); $("visibleCircleCount").textContent=filtered.length; $("circleRows").innerHTML=filtered.map(c=>`<tr><td><span class="name">${esc(c.university_name)}</span><span class="sub">${esc(c.prefecture)} ${esc(c.city||"")}</span></td><td><span class="name">${esc(c.circle_name)}</span></td><td>${badge(c.organization_type||"不明")}</td><td>${badge(statusLabel(c.verification_status),["admin_verified","university_verified"].includes(c.verification_status)?"ok":"")}</td><td>${badge(sourceLabel(c.source_type))}${c.source_url?`<span class="sub"><a href="${esc(c.source_url)}" target="_blank">出典URL</a></span>`:""}</td></tr>`).join("") || `<tr><td colspan="5" class="empty">データなし</td></tr>`; updateFilterButtons()}
