...
SOURCE_URL = "https://www.ritsumei.ac.jp/sports-culture/sports/group/"
SOURCE_MEMO = "立命館大学 SPORTS 団体紹介の体育会公認団体一覧で確認。紹介文・画像は保存せず、団体名、競技、出典URLのみ登録。"
...
OFFICIAL_CLUBS = [
    ...
    ("女子バスケットボール部", "バスケットボール"),
    ("バレーボール部", "バレーボール"),
    ("バドミントン部", "バドミントン"),
    ("ラグビー部", "ラグビー"),
    ("アメリカンフットボール部 パンサーズ", "アメリカンフットボール"),
    ("男子ラクロス部", "ラクロス"),
    ("女子ラクロス部", "ラクロス"),
    ("水泳部 競泳", "水泳"),
    ("卓球部", "卓球"),
...
def main():
    ...
    conn.execute(
        ...
        (
            ...
            "立命館大学 体育会 公認団体 主要競技",
            SOURCE_URL,
            "主要競技の体育会公認団体を大学公式ページから登録済み。文化系・任意団体・同好会は未収集。",
            timestamp[:10],
            timestamp,
        ),
    )
    conn.execute(
        ...
        """,
        (
            slug("run", f"ritsumeikan_official_sports_{timestamp}"),
            "Phase 1: 立命館大学 x 主要競技",
            "completed",
            inserted + updated,
            0,
            "大学公式の体育会公認団体一覧から主要競技のみ登録。紹介文・画像・レビュー・ランキングは未使用。",
            timestamp,
            now(),
        ),
    )
